...
void read_file(char file_name[30], struct Person *lst_persons){
    ...
    if(file){
        ...
        while (fscanf(file, "%s %d %c %lf", lst_persons[num_persons].name, 
                        &lst_persons[num_persons].age, &lst_persons[num_persons].sex, 
                        &lst_persons[num_persons].salary) == 4) {            
            num_persons++;
        }
...
void print_persons(struct Person *lst_persons){    
    cout<<left;
    cout<<setw(15)<<"Name"<<setw(10)<<"Age"<<setw(10)<<"Sex"<<setw(20)<<"Salary"<<endl;
    cout<<"---------------------------------------------"<<endl;
    ...
    if(num_persons != 0){
        for(int i = 0; i < num_persons; i++){
            cout<<setw(15)<<lst_persons[i].name<<
                    ...
                    setw(10)<<lst_persons[i].sex<<
                    setw(20)<<lst_persons[i].salary<<endl;            
        }
    }else{        
        printf("No persons\n");
    }
